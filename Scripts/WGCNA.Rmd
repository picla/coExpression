...
#samples$seqBatch <- c(rep(1, 4), rep(2, 18), rep(3, 2))
...
for (acn in unique(samples$accession))
{
  ...
  percentVar <- round(100 * attr(pcaData, "percentVar"))
  print(ggplot(pcaData, aes(PC1, PC2, color=temperature, shape=replicate)) +
  geom_point(size=3) +
  ggtitle(acn) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
...
```{r set softThreshold}
expr.filter[[1]]$softThreshold <- 20
...
for (i in 1:nSets)
{
  hist(table(expr.filter[[i]]$dynamicMods), main = names(expr.filter[i]))
}
```
...
for (i in 1:nSets)
{
  hist(table(expr.filter[[i]]$mergedColors), main = names(expr.filter[i]), breaks = 50)
}
```
...
for (i in 1:nSets) {
  expr.filter[[i]]$geneConnectivity <- intramodularConnectivity(expr.filter[[i]]$adjacency, expr.filter[[i]]$mergedColors, scaleByMax = FALSE)
  ...
  print(expr.filter[[i]]$moduleConnectivity)
}
...
```
Relative Module Quality
```{r relative module quality distribution}
for (i in 1:nSets)
{
  hist(expr.filter[[i]]$modulerelativeConnectivity$Quality.module, main = names(expr.filter[i]))
}
```
...
ClearTable<- data.frame('Name module 16C'= character(), 'Name module 6C'=character(), 'N_totgenes16'=integer(), 'N_totgenes6'=integer(), 'N_overlapped_genes'=integer(), 'p-value'=integer())

ClearTable<-Table.overlap(expr = expr.filter, pTable = pTable, Threshold = Threshold, CountTbl = CountTbl)

kable(ClearTable)
```
 ## Go enrichment for modules of interest
 Creation of a table which describes the biological processes in common (the result will be ordered by decreasing p-value) 
 
```{r Go enrichment for module in common}
ClearTable[order(-ClearTable$p.value),]
library(topGO)
...
for (i in 1:nrow(ClearTable)) {
  modulename<- c(ClearTable$NameModule6[[i]], ClearTable$NameModule16[[i]])
  Results<-list()
  for (j in 1:length(modulename)) {
    #extract genes 
    x<- modulename[[j]]
    genesinmodule<- colnames(expr.filter[[j]]$data)[expr.filter[[j]]$mergedColors == x]
    Results[[j]]<- Top.Go(genesinmodule, go_ids, genesuniverse, gene_2_GO)
  }
  #creation of a table with results
  for (Go6 in Results[[1]]$GO.ID) {
    for (Go16 in Results[[2]]$GO.ID) {
      if (Go6 == Go16) {
         lineiwant<- data.frame('Name module 16C'= modulename[[2]], 'Name module 6C'= modulename[[1]], 'GO term'= Go6, 'Function'= Results[[1]]$Term[Results[[1]]$GO.ID == Go6])
...
kable(TableGO)
#TableGO saved in Results
save(TableGO, file = '/Volumes/nordborg/pub/forPieter/WGCNA/Final Results/Table GO terms in common 6C 16C.RData')
```

I want to understand why modules overlap and check their level of expression

```{r check behaviour of modules in common}
...
for (i in 1:nrow(ClearTable)) {
  ...
  cex1 = 0.9
  for (j in 1:nSets) {
  columnname<- paste('Name.module.', names(expr.filter[j]), sep = '' )
  module<- ClearTable[,columnname][[i]]
  what.to.plot<- expr.filter[[j]]$data[colnames(expr.filter[[j]]$data) %in% colnames(expr.filter[[j]]$data)[expr.filter[[j]]$mergedColors == module]]
   Genes.behaviour(xrange = range(1:nrow(what.to.plot)), yrange = range(what.to.plot), Tableplot = what.to.plot, expr = expr.filter[j]) 
  }
}

```
 
Find specific modules for every temperature

```{r Specific modules analysis}
for (i in 1:nSets) {
  columnname<- paste('Name.module.', names(expr.filter[i]), sep = '' )
  expr.filter[[i]]$Specific.modules<- (unique(expr.filter[[i]]$mergedColors))[!((unique(expr.filter[[i]]$mergedColors)) %in% (unique(TableGO[,columnname])))]
}
```

```{r GO enrichment on specific modules}
library(topGO)
for (i in 1:nSets) {
  for (module in expr.filter[[i]]$Specific.modules) {
    genesinmodule<- colnames(expr.filter[[i]]$data)[expr.filter[[i]]$mergedColors == module]
    Result<- Top.Go(genesinmodule, go_ids, genesuniverse, gene_2_GO)
    #save the result
    write.table(Result, paste('/Volumes/nordborg/pub/forPieter/WGCNA/Final Results/GO for specific modules/', names(expr.filter[i]), '/summary_topGO_analysis_', module, '.csv', sep=''), sep=",",quote=FALSE,row.names=FALSE)
  }
}
```
