...
```{r filter genes}
# filter out genes with too many missing values and zero variance acrosse samples.
# for consensus, boht datastes need to be checked simultaneously. Use geneFilterMS.
...
for (i in 1:nSets)
{
  expr.filter[[i]]$adjacency <- adjacency(expr.filter[[i]]$data, power = expr.filter[[i]]$softThreshold);
}
```
...
for (i in 1:nSets)
{
  ...
  merge <- mergeCloseModules(expr.filter[[i]]$data, expr.filter[[i]]$dynamicColors, cutHeight = MEDissThres, verbose = 3)
  # The merged module colors
  expr.filter[[i]]$mergedColors <- merge$colors
  # translate into moduel labels
  colorOrder = c("grey", standardColors(length(table(expr.filter[[i]]$mergedColors))))
  expr.filter[[i]]$mergedLabels <- match(expr.filter[[i]]$mergedColors, colorOrder)-1
...
for (i in 1:nSets)
{
  ...
  modules <- unique(na.omit(expr.filter[[i]]$mergedLabels))
  # TODO: write function to summarize kWithin, kOut, and their ratio per module
  expr.filter[[i]]$moduleConnectivity <- lapply(modules, FUN(summarizeConnectivity))
  names(expr.filter[[i]]$moduleConnectivity) <- modules
}

# TeMP function
summarizeConnectivity <- function(module, expr)
{
  # correct the geneIdx, length is not total gene number!!!
  geneIdx <- expr$mergedLabels[expr$mergedLabels == module]
  Kwithin <- mean(expr$connectivity$kWithin[expr$mergedLabels == module])
